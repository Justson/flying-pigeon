...
//        Log.e(TAG, "returnResult:" + returnResult);
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                test(mPigeon);
                mPigeon.route("/words").withString("name", "Justson").fly();
                mPigeon.route("/hello").with(new Bundle()).fly();
                mPigeon.route("/world2").fly();
//                pigeon.route("/world/error").fly();
                mPigeon.route("/submit/bitmap", UUID.randomUUID().toString(), new byte[1024 * 1000 * 3], 1200).resquestLarge().fly();

                Integer resquestLargeResult = mPigeon.route("/submit/bitmap2", UUID.randomUUID().toString(), new byte[1024 * 1000 * 3], 1200).resquestLarge().<Integer>fly();
                Log.e(TAG, "resquestLargeResult:" + resquestLargeResult);
                byte[] data = mPigeon.route("/query/bitmap", "girl.jpg", 5555).responseLarge().fly();
                if (null != data) {
